...
async def handle_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_staff(uid):
        ...
        if 'медiафайли' in tlo or 'медіафайли' in tlo:
            logs = list(ctx.bot_data.get('media_log', []))
            ...
            lines = ['Iсторiя медiафайлiв:']
            for item in logs[:50]:
                lines.append('{} | {} | {} | {}'.format(
                    item['ts'].strftime('%d.%m.%y %H:%M'),
                    item.get('media_type') or 'Медiа',
                    item.get('phone') or 'не вказано',
                    item.get('name') or 'Клiєнт'
                ))
...
async def handle_media(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    log_media_sheet(uid, detect_media_type(msg))
...
def log_media_sheet(uid, media_type):
    try:
        ws = get_ws('Медиа')
        ws.append_row([str(uid), media_type, now_str()])
    except Exception as e:
