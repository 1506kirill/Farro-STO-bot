...
async def handle_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if (not is_staff(uid)) and (not has_registered_phone(uid)):
        if ud.get('reg_step') != 'phone':
            ud['reg_step'] = 'phone'
            ud['reg_data'] = {}
        # дозволяємо лише етап введення номера; будь-який iнший текст не пiде менеджеру
        if ud.get('reg_step') != 'phone':
...
async def handle_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('write_'):
        ...
        # якщо номер уже є, але iм'я/авто ще не зiбрали в рамках нового сценарiю
...
async def handle_contact(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    data = ud.setdefault('reg_data', {})
    data['phone'] = phone
    ud['reg_step'] = 'choose_service'
    await update.message.reply_text('Оберiть, куди хочете написати:', reply_markup=kb_write())
