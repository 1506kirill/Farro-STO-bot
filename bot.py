    ...
    try:
        r = claude_client.messages.create(
            ...
            messages=[{'role':'user','content':(
                ...
                '. Переклади це однiєю вiдповiддю українською мовою. '
                'Збережи змiст максимально точно, не змiнюй сенс, '
                'але зроби текст трохи бiльш природним, ввiчливим i виразним. '
...
async def forward_media_to_client(bot, client_id, source_chat_id, message_id):
    await bot.forward_message(chat_id=int(client_id), from_chat_id=source_chat_id, message_id=message_id)
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data.clear()
    if is_staff(uid):
        await update.message.reply_text('Привiт, {}! ID: {}'.format(name,uid), reply_markup=kb_staff(get_unread_count(ctx, uid))); return
    client = get_client(uid)
...
async def handle_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_staff(uid):
        if 'новi' in tlo or 'заявки' in tlo:
            ...
            if not new_r:
                await update.message.reply_text('Нових заявок немає.', reply_markup=kb_staff(get_unread_count(ctx, uid))); return
            lines = ['Новi заявки: {}'.format(len(new_r))]
            for r in new_r:
                lines.append('{} | {} | {} | {}'.format(r[0],r[3],r[7],r[1]))
            await update.message.reply_text('\n'.join(lines), reply_markup=kb_staff(get_unread_count(ctx, uid))); return

        if 'активн' in tlo or 'всi' in tlo:
            ws     = get_ws('Заказы')
            active = [r for r in ws.get_all_values()[1:] if len(r)>9 and r[9] not in ('issued','')]
            if not active:
                await update.message.reply_text('Активних немає.', reply_markup=kb_staff(get_unread_count(ctx, uid))); return
        ...
        if 'клiєнти' in tlo or 'клієнти' in tlo:
            ...
            if not clients:
                await update.message.reply_text('Клiєнтiв немає.', reply_markup=kb_staff(get_unread_count(ctx, uid))); return
            btns = [[InlineKeyboardButton(
        ...
        await update.message.reply_text('Оберiть дiю:', reply_markup=kb_staff(get_unread_count(ctx, uid))); return
...
async def handle_media(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_staff(uid):
        if ud.get('reply_to'):
            cid = ud.pop('reply_to')
            try:
                await forward_media_to_client(ctx.bot, cid, msg.chat_id, msg.message_id)
            except Exception as e:
                logger.error('forward media to client: %s', e)
                await msg.reply_text('Не вдалося переслати медiафайл клiєнту.', reply_markup=kb_staff(get_unread_count(ctx, uid)))
            return
        await msg.reply_text('Оберiть клiєнта i натиснiть «↩️ Вiдповiсти», перш нiж надсилати медiафайли.', reply_markup=kb_staff(get_unread_count(ctx, uid)))
        return
