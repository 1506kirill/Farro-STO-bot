...
async def handle_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Менеджер: команди ─────────────────────────────────────
    if is_staff(uid):
        if 'контакти' in tlo or 'контакты' in tlo:
            c_s = CONTACTS['sto']
            c_b = CONTACTS['body']
            lines = [
                'Контакти:',
                '',
                '🚗 {}'.format(c_b['name']),
                '{} {}'.format(c_b['address'], c_b['note']),
                'Графiк: {}'.format(c_b['hours']),
                'Маршрут: {}'.format(c_b['maps']),
                '',
                '🔧 {}'.format(c_s['name']),
                '{}'.format(c_s['address']),
                'Графiк: {}'.format(c_s['hours']),
                'Маршрут: {}'.format(c_s['maps']),
                '',
                'Телефони:',
                PHONES
            ]
