...
BODY_MANAGER_IDS = [int(x.strip()) for x in os.environ.get('BODY_MANAGER_IDS','').split(',') if x.strip()]  # кузовний сервiс
STO_MANAGER_IDS  = [int(x.strip()) for x in os.environ.get('STO_MANAGER_IDS','').split(',') if x.strip()]   # СТО
CLAUDE_API_KEY = os.environ.get('CLAUDE_API_KEY')

STAFF_IDS     = list(set(MASTER_IDS + BODY_MANAGER_IDS + STO_MANAGER_IDS + ([OWNER_ID] if OWNER_ID else [])))
...
def get_staff_recipients(sto_key=None):
    if sto_key == 'body' and BODY_MANAGER_IDS:
        return BODY_MANAGER_IDS
    if sto_key == 'sto' and STO_MANAGER_IDS:
        return STO_MANAGER_IDS
    return STAFF_IDS

async def to_staff(bot, msg, client_id=None, sto_key=None):
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(
        '↩️ Вiдповiсти', callback_data='reply_{}'.format(client_id))]]) if client_id else None
    for uid in get_staff_recipients(sto_key):
        try: await bot.send_message(chat_id=uid, text=msg, reply_markup=kb)
        except Exception as e: logger.error('staff %s: %s', uid, e)

async def forward_media_to_staff(bot, source_chat_id, message_id, header, client_id=None, sto_key=None):
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(
        '↩️ Вiдповiсти', callback_data='reply_{}'.format(client_id))]]) if client_id else None
    for uid in get_staff_recipients(sto_key):
        try:
...
async def handle_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_staff(uid):
        if 'клiєнти' in tlo or 'клієнти' in tlo:
            ...
            for r in rows:
                if len(r) < 2:
                    continue
                try:
                    dt = datetime.strptime(r[1], '%d.%m.%y %H:%M')
                except:
                    continue
                if dt.timestamp() >= cutoff:
                    uid_c = r[0]
                    client = get_client(uid_c) or {}
                    data.append((
                        dt,
                        client.get('name') or 'Клiєнт',
                        client.get('phone') or uid_c,
            ...
            for dt, name, phone, car in data[:50]:
                lines.append('{} | {} | {} | {}'.format(
        ...
        if 'чати' in tlo:
            rows = get_ws('Чаты').get_all_values()[1:]
            if not rows:
                await update.message.reply_text('Чатiв поки немає.', reply_markup=kb_staff()); return
            ...
            for r in rows:
                ...
                uid_c = r[0]
                try:
                    dt = datetime.strptime(r[2], '%d.%m.%y %H:%M')
                except:
                    continue
                if uid_c not in latest or dt > latest[uid_c]['dt']:
                    client = get_client(uid_c) or {}
                    latest[uid_c] = {
                        'dt': dt,
                        'name': client.get('name') or 'Клiєнт',
                        'phone': client.get('phone') or uid_c,
                        'car': client.get('car') or 'не вказано'
                    }

            items = sorted(latest.items(), key=lambda x: x[1]['dt'], reverse=True)[:30]
            btns = [[InlineKeyboardButton(
                '{} | {} | {}'.format(v['dt'].strftime('%d.%m %H:%M'), v['name'], v['phone']),
                callback_data='chat_{}'.format(k)
            )] for k, v in items]
            btns.append([InlineKeyboardButton('Скасувати', callback_data='cancel')])
            await update.message.reply_text('Оберiть чат:', reply_markup=InlineKeyboardMarkup(btns)); return
    ...
    await to_staff(ctx.bot, fwd, client_id=uid, sto_key=sto)
...
async def handle_media(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await forward_media_to_staff(ctx.bot, msg.chat_id, msg.message_id, header, client_id=uid, sto_key=sto)
...
async def handle_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('call_'):
        ...
        await to_staff(ctx.bot, msg, client_id=uid, sto_key=sto_key)
        await replace_or_send_text(
            q, ctx,
    ...
    if data.startswith('chat_'):
        cid = data[5:]
        chat_rows = get_chat(cid)
        if not chat_rows:
            await replace_or_send_text(q, ctx, 'Iсторiя чату порожня.'); return

        lines = []
        for r in chat_rows[-20:]:
            direction = r[1] if len(r) > 1 else ''
            dt = r[2] if len(r) > 2 else ''
            body = r[3] if len(r) > 3 else ''
            who = 'Клiєнт' if direction == 'client' else 'Менеджер'
            lines.append('{} | {}\n{}'.format(dt, who, body))

        ud['reply_to'] = int(cid)
        text_chat = 'Iсторiя чату:\n\n' + '\n\n'.join(lines)
        if len(text_chat) > 3900:
            text_chat = text_chat[-3900:]
        await replace_or_send_text(q, ctx, text_chat)
        return
